...
const LibraryItem = (props: LibraryItemProps) => {
  ...
  return (
    <section
      className='grid md:grid-cols-2 items-center lg:flex-row w-full'>
      {items.map(item => <Item key={item.title} type={item.type} title={item.title} desc={item.desc} link={item.link} linkText={item.linkText} />)}
    </section>
  )
}
