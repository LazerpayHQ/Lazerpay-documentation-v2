...
const CodeSelect = (props: SelectProps) => {
  ...
  return (
    <div className='relative'>
      <button onClick={() => setVisible(prev => !prev)} className='text-neu-700 caption-s rounded-8 px-2.5 py-1.5 text-xs flex items-center h-fit bg-neu-100 outline-none'>
        <span className='mr-1.5 leading-4'>{items[selected]}</span>
        <ArrowDown />
      </button>
      <ul ref={ref} className={cn(Styles.select, { [Styles.selectActive]: visible }, 'list-none m-0 p-0 bg-neu-100 rounded-lg p-3.5 text-xs font-proxima w-max absolute z-[6]')}>
        {items.map((item, i) => (
          <li key={item} className='mb-1.5'>
            <button type='button' aria-label={item} data-value={i} onClick={onSelected} className={cn('w-full py-0.5 px-2.5 text-left rounded', {
